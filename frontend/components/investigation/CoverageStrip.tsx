...
export function CoverageStrip({ capability }: { capability: CapabilityState }) {
  ...
  const reasons = [...new Set(coverage?.provider_errors?.map(error => readable(error.code).toUpperCase()) || [])];
  ...
  return <details className={`coverage-strip ${coverage?.partial || capability.processing_state === 'failed' ? 'coverage-limited' : ''}`}>
    ...
    <div className="coverage-details">
      ...
      {reasons.length > 0 && <p><strong>Observation stopped:</strong> {reasons.join(', ')} · {coverage?.provider_requests ?? 'Unknown'} provider requests</p>}
